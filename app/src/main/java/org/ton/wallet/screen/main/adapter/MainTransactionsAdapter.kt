...
class MainTransactionsAdapter(
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        if (holder is TransactionItemViewHolder) {
            holder.bind(getItemAt(position) as TransactionItem)
        } else if (holder is TransactionHeaderViewHolder) {
            holder.bind(getItemAt(position) as HeaderItem)
        }
    }
    ...
    override fun getItemViewType(position: Int): Int {
        return when (getItemAt(position)) {
            is HeaderItem -> ViewTypeHeader
            else -> ViewTypeTransaction
        }
    }

    fun setItems(items: List<Any>, withAnimation: Boolean) {
        items.forEach {
            if (it is BaseItem) {
    ...
    abstract class BaseItem(var isVisible: Boolean = true)

    class HeaderItem(val title: String) : BaseItem()
    ...
    private abstract class BaseViewHolder<T : BaseItem>(view: View) : RecyclerHolder<T>(view) {
    ...
    private class TransactionHeaderViewHolder(parent: ViewGroup) : BaseViewHolder<HeaderItem>(TextView(parent.context)) {
    ...
    private class TransactionItemViewHolder(
    ...
    ) : BaseViewHolder<TransactionItem>(ConstraintLayout(parent.context)), View.OnClickListener {
        ...
        init {
            ...
            constraintLayout.foreground = Res.drawable(R.drawable.ripple_rect)
            constraintLayout.setBackgroundResource(R.drawable.bkg_transaction)
            ...
            imageView.setImageResource(R.drawable.ic_gem_18)
            ...
            messageText.setBackgroundResource(R.drawable.bkg_transaction_msg)
        ...
        override fun bind(item: TransactionItem) {
            ...
            peerTypeText.text =
                ...
                else if (item.type == TransactionDto.Type.In) Res.str(R.string.from)
                else Res.str(R.string.to)
    ...
    interface TransactionItemCallback {
        ...
        fun onTransactionClicked(transaction: TransactionItem)
    }
