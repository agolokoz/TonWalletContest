...
import org.ton.wallet.lib.core.ext.containsMotionEvent
...
abstract class BaseBottomSheetController @JvmOverloads constructor(
    ...
    private fun show() {
        if (isAnimatedOpen) {
            val widthSpec = MeasureSpec.makeMeasureSpec(Res.screenWidth, MeasureSpec.AT_MOST)
            val heightSpec =
                if (isFullHeight) MeasureSpec.makeMeasureSpec(Res.screenHeight, MeasureSpec.EXACTLY)
                else MeasureSpec.makeMeasureSpec(Res.screenHeight, MeasureSpec.AT_MOST)
    ...
    private inner class BottomSheetLayout(context: Context) : FrameLayout(context) {
        ...
        init {
            val bottomSheetDrawable = Res.drawable(R.drawable.sheet_shadow_round)
