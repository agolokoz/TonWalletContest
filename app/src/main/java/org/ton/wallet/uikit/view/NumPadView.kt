...
class NumPadView @JvmOverloads constructor(
    ...
    init {
        ...
        try {
            isDarkTheme = typedArray.getBoolean(R.styleable.NumPadView_numpad_dark, isDarkTheme)
            withDot = typedArray.getBoolean(R.styleable.NumPadView_numpad_dot, withDot)
        } finally {
            typedArray.recycle()
        }
    ...
    fun setDark(isDark: Boolean) {
        val drawableColor =
            if (isDark) Res.color(R.color.numpad_button_image_tint_dark)
            else Res.color(R.color.numpad_button_image_tint_light)
        forEachIndexed { index, view ->
            (view as? ButtonView)?.let { v ->
                v.setDark(isDark)
                if (index == PositionBackSpace) {
                    v.setDrawable(Res.drawableColored(R.drawable.ic_delete, drawableColor))
                }
            }
        }
    }
