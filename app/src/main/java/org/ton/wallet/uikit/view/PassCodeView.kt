...
class PassCodeView @JvmOverloads constructor(
    ...
    init {
        ...
        try {
            isDarkTheme = typedArray.getBoolean(R.styleable.PassCodeView_passcode_dark, isDarkTheme)
        } finally {
            typedArray.recycle()
        }
        setDark(isDarkTheme)
    ...
    fun setDark(isDark: Boolean) {
        defaultBackgroundPaintColor =
            if (isDark) Res.color(R.color.passcode_background_dark)
            else Res.color(R.color.input_disabled)
        defaultFillPaintColor =
            if (isDark) Res.color(R.color.common_white)
            else Res.color(R.color.common_black)

        backgroundPaint.color = defaultBackgroundPaintColor
        foregroundPaint.color =
            if (isDark) Res.color(R.color.common_black)
            else Res.color(R.color.common_white)
        fillPaint.color = defaultFillPaintColor
    }
